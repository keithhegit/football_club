...
export const MatchView: React.FC<MatchViewProps> = ({ homeTeam, awayTeam, onMatchComplete, userTeamId }) => {
    ...
    if (matchState === MatchState.PLAYING) {
      interval = setInterval(() => {
        setMinute(prev => {
          ...
          const event = simulateMinute(newMinute, homeTeam, awayTeam, scores);
          
          if (event) {
            setEvents(prevEvents => [...prevEvents, event]);
            if (event.type === 'GOAL') {
               if (event.teamId === homeTeam.id) setScores(s => ({ ...s, home: s.home + 1 }));
  ...
  }, [matchState, speed, homeTeam, awayTeam, scores, headline]);
