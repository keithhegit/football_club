...
const AttributeRow = ({ label, value }: { label: string, value: number }) => {
  ...
  return (
    <div className="flex justify-between items-center text-xs py-0.5 border-b border-slate-800/50">
...
export const PlayerProfileCard: React.FC<Props> = ({ player }) => {
  ...
  return (
    <div 
        ...
        className="relative h-80 w-full cursor-pointer perspective-1000 group select-none"
    >
        <div className={`relative w-full h-full duration-500 preserve-3d transition-transform ${flipped ? 'rotate-y-180' : ''}`}>
            ...
            <div className="absolute w-full h-full backface-hidden bg-slate-800 rounded-xl border border-slate-700 shadow-xl overflow-hidden flex flex-col">
                {/* Header */}
                <div className="h-20 bg-gradient-to-r from-slate-900 to-slate-800 p-4 flex justify-between items-start">
                    <div>
                        <h3 className="font-bold text-white text-lg leading-none truncate w-32">{player.name}</h3>
