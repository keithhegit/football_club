...
### 2.4 行动纲领（近期迭代计划，new_league 分支执行）

1. **状态快照与换人管线（落地）**  
   - 引擎每整分钟记录体能/士气/condition 快照并随 `MatchResult` 返回；UI 播放按分钟展示实时状态（移除 100/75 常量）。  
   - 战术面板显示首发 + 替补，标识身份；接入 3 次换人流程（赛前/半场/比赛中皆可），写入事件日志。  
2. **传球/抢断数值再校准（Guide to FM 对齐）**  
   - 事件权重：PASS_SHORT/Long 升权、DRIBBLE 适度 ↑、TACKLE ↓、INTERCEPT/CLEARANCE ↑；tick 6-8s。  
   - 成功率：传球采用 Passing*0.4 + Vision*0.2 + Decisions*0.2 + Technique*0.2，乘以战术/状态/情境/运气；TACKLE 成功率受对手 Dribbling/Balance 抵消，仅记录成功抢断。目标传球成功率 50-95%、抢断 20-40/队。  
3. **战术映射强化**  
   - Tempo/Directness/Pressing/Mentality/Width 对事件分布和成功率的乘子表入库，遵循 Guide to FM；高压提高抢断权重但体能消耗与犯规上升。  
4. **UI 与交互**  
   - 半场自动弹出战术面板；倍速默认 1x，x1/x2/x4 + Pause。  
   - 事件视图与统计保持实时：控球、xG、射门、传球(成功率)、抢断、犯规/牌、角球/任意球、Key Players。  
5. **数据一致性与验证**  
   - Schema 与 SortItOutSI 对齐；snake_case/PascalCase 映射。  
   - 回归校验：跑批次模拟（≥100 场先行，后续 1000 场基线）对进球/传球成功/抢断/犯规/xG 分布做偏差检查。  

### 2.5 当前冲刺（执行序）
1) 落地状态快照 → UI 实时体能/士气 → 接入换人流程（new_league）。  
2) 调整事件权重/成功率与 tick，压降抢断，抬升传球数量与成功率。  
3) 校验战术乘子表（Tempo/Directness/Pressing/Mentality），对照 Guide to FM 进行数值微调。  
4) 回归抽测并记录分布，必要时继续微调。  
...
- 已实装：临场战术面板常驻（赛前/半场/全场可呼出），查看体能/士气/位置，支持节奏倍速 1x/2x/4x 与暂停；返回 Dashboard 按钮。  
- 已实装：比赛半场自动弹出战术面板；战术入口移动到比分下方“战术部署”按钮；暂停/倍速置右侧。  
- 已实装：战术面板仅展示己方球员状态（体能/士气进度条），首发限制 11 人参与模拟。  
- 已实装：回到 Dashboard 通过 onMatchComplete 流程保存并返回，避免中途跳登录。  
- 已实装：ClubLogo 优先按 clubName 匹配，兼容本地存档自定义 id，队徽回归。  
- 数值调优：防守抢断权重下调、tick 放慢（7-11s）、传球占比上调，仅记录成功抢断，持续压低抢断/犯规过高问题。  
...
### 3.3 赛季循环与比赛轮次 (Phase 4 规划)
- 联赛轮次公式：若联赛有 N 支球队，双循环赛制总轮次 = (N - 1) * 2，单循环 = (N - 1)。每轮包含 N/2 场比赛。  
  - 例：英超/西甲 N=20 → 38 轮，每队 38 场。  
- 赛季节奏：周为单位推进，典型一周一赛（含少量双赛周）。赛程应在赛季开始前生成完整轮次；已赛轮次需持久化到本地存档。  
- Phase 4 工作项：  
  1) 日程生成：按 (N-1)*2 生成全季 Fixtures，支持主客对调。  
  2) 赛程持久化：存入本地存档/IndexedDB，避免刷新丢失；支持进度断点续赛。  
  3) 多赛周兼容：允许双赛周插入杯赛/补赛（占位设计，可后续扩展）。  
  4) 赛后积分/积分榜更新：比赛结束立即写入存档并刷新联赛榜。  
...
### 🔮 Phase 3: 高级战术系统 (待定/规划中)
*   [ ] **战术克制逻辑**: 阵型克制矩阵（如 4-3-3 宽度克制 4-4-2 窄中场），提供提示并进入引擎权重。
*   [ ] **角色互补/冲突**: 如 `DLP` → `AF` 传球权重加成，边路宽度冲突惩罚；表驱动可配置。
*   [ ] **动态 AI**: 对手教练随比分/时间动态调整心态与指令。
*   [ ] **战术界面完善**: FM 式阵型/角色/职责、团队指令/对抗指令、替补/换人流程，赛前/半场自动弹出，比赛中可随时调，设置可带入比赛。
